...
def process_song_data(spark, input_data, output_data):
    ...
    songs_table.write.partitionBy('artist_id', 'year').parquet("s3a://udacity-bucket-demo-sandhya/tables/Song_Table",
                                                               mode='overwrite')

    # extract columns to create artists table
    artists_table = df.select(df['artist_id'], df['artist_name'], df['artist_location'], df['artist_latitude'],
                              df['artist_longitude'])
...
def process_log_data(spark, input_data, output_data):
    ...
    #      read log data file
    df = spark.read.format('json').load('s3a://udacity-dend/log_data/2018/11/2018-11-13-events.json')
